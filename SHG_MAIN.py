...
def UI_SerBrowse(WorkDir):
    ...
    return FileNames, values['-DX-'], values['-DISP-'], None if values['-RATIO-'] == '' else values[
        '-RATIO-'], None if values['-SLANT-'] == '' else values['-SLANT-'], values['-FIT-'], values['-CLAHE_ONLY-']
...
if len(serfiles) == 0:
    serfiles, shift, flag_display, ratio_fixe, slant_fix, save_fit, clahe_only = UI_SerBrowse(
        WorkDir)  # TODO as options is defined as global, only serfiles could be returned
    try:
        shift_choice = shift.split(':')
        if len(shift_choice) == 1:
    ...
    try:
        options['ratio_fixe'] = float(
            ratio_fixe) if ratio_fixe is not None else None
    except ValueError:
        print('invalid ratio_fixe value')
        sys.exit()
    try:
        options['slant_fix'] = float(
            slant_fix) if slant_fix is not None else None
    except ValueError:
    ...
    for serfile in serfiles:
        ...
        try:
            ...
            for frame, shift in zip(frames, options['shift']):
                ...
                if not cercle == (-1, -1, -1) and disk_display:
                    x0 = int(cercle[0])
                    y0 = int(cercle[1])
                    r = int(cercle[2])
                    frame_contrasted3 = cv2.circle(
                        frame_contrasted3, (x0, y0), r, 80, -1)
                ...
                # Modification Jean-Francois: placed before the IF for clear
                # reading
                cv2.imwrite(basefich + '_clahe.png', cc)
                if not options['clahe_only']:
                    # sauvegarde en png pour appliquer une colormap par autre
                    # script
                ...
                if flag_result_show:
                    ...
                    screen.destroy()
                    scale = min(
                        screensize[0] / im_3.shape[1],
                        screensize[1] / im_3.shape[0])
                    cv2.namedWindow('Sun images', cv2.WINDOW_NORMAL)
                    cv2.moveWindow('Sun images', 0, 0)
                    cv2.resizeWindow('Sun images',
                                     int(im_3.shape[1] * scale),
                                     int(im_3.shape[0] * scale))
